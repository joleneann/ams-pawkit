...
- **Streaming upgrade pending.** Sarvam Saarika WebSocket streaming was requested 2026-05-18 (Discord + email `developer@sarvam.ai`). Account currently lacks streaming entitlement (403 on all endpoints). Once enabled, swap the batch transcribe call for a streaming WebSocket connection. Same UI architecture; live transcript replaces post-stop transcribe.
