...
/**
 * Pawkit Sheet (side-anchored Dialog variant, shadcn-style).
 * Built on Radix Dialog. Used for: parent app override picker (right side),
 * dashboard quick-action panels (right side), filter drawers (left side).
 *
 * Shipped here even though the parent app's override picker uses a native
 * `@gorhom/bottom-sheet` per `docs/reusable-tooling.md` — the Sheet
 * primitive is for the dashboard side surfaces.
 */
  ...
  "fixed z-50 gap-4 bg-card p-6 shadow-lg transition ease-in-out data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:duration-300 data-[state=open]:duration-500",
  {
    variants: {
      side: {
        ...
        bottom:
          "inset-x-0 bottom-0 border-t border-ink/10 data-[state=closed]:slide-out-to-bottom data-[state=open]:slide-in-from-bottom",
        ...
        right:
          "inset-y-0 right-0 h-full w-3/4 border-l border-ink/10 data-[state=closed]:slide-out-to-right data-[state=open]:slide-in-from-right sm:max-w-sm",
      },
    },
    defaultVariants: {
      side: "right",
